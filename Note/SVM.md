...
在支持向量机中，对偶问题不管是目标函数还是决策函数都涉及到输入实例与实例之间的内积，可以用和函数$K(x_i, x_j) = \phi(x_i) \phi(x_j)$来代替。目标函数和决策函数就变为：

$$
\begin{aligned}
  W(\alpha) &= \frac{1}{2} \sum_{i=1}^{N} \sum_{j=1}^{N} \alpha_i \alpha_j y_i y_j K(x_i,x_j) - \sum_{i=1}^{N} \alpha_i \\
  f(x)  &= sign(\sum_{i=1}^{N_s} \alpha_i^* y_i \phi(x_i) \phi(x) + b^*) \\
        &= sign(\sum_{i=1}^{N_s} \alpha_i^* y_i K(x_i, x) + b^*)
\end{aligned}
$$

也就是通过映射函数$\phi$将原来的输入空间变换到另一个特征空间，输入空间的内积变成新空间的内积，在新空间中训练样本学习支持向量机。也就是在核函数给定的条件下，利用解线性分类问题的方法求解非线性分类问题。学习是隐式的在特征空间中进行，不需要显式地定义特征空间和映射函数。

#### 常用核函数

大概常用的核函数有这么几个：**多项式核函数（polynomial kernel function）**、**高斯核函数（Gaussian kernel function）**、**字符串核函数（string kernel function）**。
...
1. 多项式核函数
    ...
    对应的支持向量机是一个p次多项式分类器，分类决策函数变成了：

    $$
      f(x) = sign(\sum_{i=1}^{N_s} \alpha_i^* y_i(x_i x + 1)^p + b^*)
    $$
2. 高斯核函数
    ...
    $$
      f(x) = sign(\sum_{i=1}^{N_s} \alpha_i^* y_i \exp(- \frac{||x-x_i||^2}{2\sigma^2}) + b^*)
    $$
3. 字符串核函数

    核函数不仅是可以定义在欧氏空间上，也可以定义在离散数据的集合上。字符串核就是定义在字符串集合上的核函数。字符串核函数应用于文本分类、信息检索、生物信息等方面。
    ......
...
#### 序列最小化算法（sequential minimal optimization）

当训练样本容量很大的时候，SVM算法就非常低效，难以使用。通过**SMO算法**可以高效实现SVM的学习。
